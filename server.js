...
// Trust proxy (required when behind ngrok/reverse proxy/Vercel)
...
// Session configuration — use PostgreSQL store so sessions survive across
// serverless function invocations on Vercel (in-memory store loses data)
app.use(session({
    store: new pgSession({
        pool: pgPool,
    ...
    cookie: {
        ...
        httpOnly: true,
        sameSite: 'lax',              // ensure cookie is sent on same-site navigations
        maxAge: 7 * 24 * 60 * 60 * 1000 // 7 days (was 24h — too short)
    }
}));
